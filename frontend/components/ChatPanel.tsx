...
export default function ChatPanel() {
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", text: 'Ask me things like "is it safe to go jogging at 5pm?"' },
  ]);
  ...
  async function send() {
    ...
    try {
      ...
      setMessages((m) => [
        ...m,
        { role: "assistant", text: data.reply ?? data.message ?? "Something went wrong." },
      ]);
    } catch {
      setMessages((m) => [...m, { role: "assistant", text: "Couldn't reach the assistant — try again shortly." }]);
    } finally {
      setSending(false);
    }
  }
  ...
  return (
    <div className="flex h-full flex-col rounded-instrument border border-haze-50 bg-panelRaised shadow-instrument">
        ...
        {messages.map((m, i) => (
          <div
            ...
            className={`max-w-[85%] rounded-instrument px-3 py-2 text-sm ${
              m.role === "user" ? "ml-auto bg-brand text-white" : "bg-panelSunken text-ink"
            }`}
          >
            {m.text}
          </div>
